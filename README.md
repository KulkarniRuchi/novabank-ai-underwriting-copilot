...
SHAP Explainability (global + per-customer)
        │
        ▼
AI Underwriting Copilot (LLM-generated officer report via Ollama, with rule-based fallback)
```
...
### Enabling LLM-generated reports (Ollama — free & fully local)

The underwriting reports are generated by a local open-source LLM served by
[Ollama](https://ollama.com) — no API key, no cloud, no cost. Setup:
...
The copilot auto-detects a running Ollama server. To use a different model or host:

```bash
export OLLAMA_MODEL=mistral            # default: llama3.1
export OLLAMA_BASE_URL=http://localhost:11434
```

If Ollama isn't running, a deterministic **rule-based generator** produces an
equivalent report, so the whole system works with no LLM at all.
...
Feeds the customer profile, predicted PD, and top SHAP drivers into a **local open-source
LLM (via Ollama)**, which returns a structured report: **Risk Level**, **Recommendation**,
**Key Reasons** (grounded in the actual SHAP values — the prompt explicitly forbids
inventing factors), and **Suggested Improvements**. Running the LLM locally means zero
API cost and no customer data ever leaves the machine — a meaningful property in a
banking context.

If Ollama isn't available, a deterministic rule-based generator produces an equivalent
(if less fluent) report, so the whole system works with no LLM at all.

## Tech Stack

Python · NumPy · Pandas · Scikit-learn · XGBoost · SHAP · Streamlit · Ollama (local LLM) · Faker
